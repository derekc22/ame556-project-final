...
TASK 2c: BACKWARD WALKING
============================================================================

Parameters: freq=1.2Hz, amp=10 deg, lean=-8 deg, Kp=80, Kd=20
Results: 10s, -2.429m backward, 0.243 m/s

Requirements from PDF:
- Walking backward with speed >= 0.5 m/s
- Duration >= 5 seconds
...
def check_constraints(q, dq):
    ...
    Per Task 1 requirements, simulation should terminate if any constraint is violated.

    Physical Constraints from PDF:
    - Hip angles: -120 deg <= q <= 30 deg  (or -2.094 rad to 0.524 rad)
    - Knee angles: 0 deg <= q <= 160 deg   (or 0 rad to 2.793 rad)
    - Hip velocities: |dq| <= 30 rad/s
    - Knee velocities: |dq| <= 15 rad/s

    Args:
        q: Joint angles [q1, q2, q3, q4] in radians
        dq: Joint velocities [dq1, dq2, dq3, dq4] in rad/s

    Returns:
        (violated, message): Tuple of (bool, str) - True if constraint violated
    """
...
def generate_plots(log, x0, distance, speed, final_t, output_dir):
    ...
    fig.suptitle(f'Task 2c: Backward Walking - Constraint Verification\n'
    ...
    ax.set_title('COM X Position vs Time (Negative = Backward)')
    ax.legend(loc='best')
    ...
    ax.set_title('COM X Velocity vs Time (Negative = Backward)')
    ax.legend(loc='best')
    ax.grid(True, alpha=0.3)
    ...
    # Save plot
    plot_path = output_dir / "task2c_plots.png"
...
def run_backward_walk(xml_path, duration=10.0, video_name="task2c_backward.mp4"):
    """
    ...
    - Knee lift for ground clearance during swing
    ...
    q1 = np.deg2rad(-50)  # Left hip
    q2 = np.deg2rad(40)  # Left knee
    q3 = np.deg2rad(-5)  # Right hip
    q4 = np.deg2rad(55)  # Right knee
    ...
    print(f"Initial pitch: {np.rad2deg(data.qpos[2]):.1f} deg")

    # -------------------------------------------------------------------------
    # WALKING GAIT PARAMETERS (TUNED FOR BACKWARD WALKING)
    ...
    # Knee lift for ground clearance
    ...
    print(f"  Hip amplitude: {np.rad2deg(amp):.0f} deg")
    print(f"  Lean: {np.rad2deg(lean):.0f} deg")
    ...
    while data.time < duration:
        ...
        if abs(pitch) > np.deg2rad(60):
            print(f"\nFELL at t={t:.2f}s (pitch={np.rad2deg(pitch):.1f} deg)")
            fell = True
            break

        # Check physical constraints (terminate if violated per Task 1 requirement)
        violated, msg = check_constraints(q, dq)
        if violated:
            print(f"\nCONSTRAINT VIOLATION at t={t:.2f}s: {msg}")
            break

        # ---------------------------------------------------------------------
        # GAIT GENERATION: Sinusoidal hip oscillation with knee lift
        # ---------------------------------------------------------------------
        ...
        # Hip oscillation pattern that produces backward motion
        q_des[0] = q0[0] + lean - amp * np.sin(phase)  # Left hip
        q_des[2] = q0[2] + lean + amp * np.sin(phase)  # Right hip

        # Knee lift for clearance during swing phase
        q_des[1] = q0[1] + knee_lift * max(0, -np.sin(phase))  # Left knee
        q_des[3] = q0[3] + knee_lift * max(0, np.sin(phase))  # Right knee
        ...
        if step % int(1.0 / dt) == 0:
            vx = data.qvel[0]
            print(f"  t={t:.1f}s: x={x_pos:.3f}m, pitch={np.rad2deg(pitch):.1f} deg, vx={vx:.2f}m/s")
    ...
    print("=" * 60)
    print(f"Duration >= 5s: {final_t:.2f}s {'PASS' if final_t >= 5.0 else 'FAIL'}")
    print(f"Speed >= 0.5 m/s: {speed:.3f} m/s {'PASS' if speed >= 0.5 else 'FAIL'}")
...
if __name__ == "__main__":
    ...
    # Run simulation
    result = run_backward_walk(str(XML_PATH), duration=10.0, video_name="task2c_backward.mp4")

    print(f"\nOutput files in: {OUTPUT_DIR}")
    print(f"  - task2c_backward.mp4 (video)")
    print(f"  - task2c_plots.png (constraint verification)")